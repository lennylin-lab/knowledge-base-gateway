...
Wrap errors with operation context while preserving cancellation. Known model that exists in the catalog but the subject lacks permission, and unknown models, collapse to the same non-leaky 403 `model_not_allowed` — never distinguish them in the response. Retry only pre-output network errors, 429 and 5xx under one total deadline; never retry after SSE starts. Client cancellation stops upstream work. Log classification, request ID, model and latency, never authorization headers, raw bodies, secrets, SQL values or internal URLs.
